...
const PdfViewer = ({
  ...
  url: string;
  addNewHighlight: (highlight: GhostHighlight) => void;
}) => {
  ...
  useEffect(() => {
    // console.log("url :: ", url);
    setHighlights(TEST_HIGHLIGHTS[url] ?? []);
  }, [url]);
  ...
  const createCenteredHighlight = () => {
    const ghostHighlight: GhostHighlight = {
      content: {
        text: "New Highlight",
      },
      position: {
    ...
    addNewHighlight(ghostHighlight);
  };
